...
export default function UsersTable({
  ...
  const makePagination = (): React.ReactNode[] => {
    ...
    const enabledClass = "bg-[#267A4C]/10 text-[#1C5C39] border-[#267A4C]/20 hover:bg-[#267A4C]/20";
    const activeClass = "bg-[#267A4C] text-white border-[#267A4C] shadow";
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="flex items-center gap-3 text-[#1B4A31]">
          <div className="h-6 w-6 animate-spin rounded-full border-2 border-[#267A4C] border-t-transparent"></div>
  ...
  if (!users.length) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#267A4C]/10 mb-4">
          <User className="h-8 w-8 text-[#267A4C]" />
        </div>
        <h3 className="text-lg font-semibold text-[#142A1C] mb-2">No users found</h3>
        <p className="text-sm text-[#1B4A31]">Start by adding your first user to the system.</p>
      </div>
    );
  }
        ...
        >
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-4 flex-1 min-w-0">
              <Avatar className="h-12 w-12 ring-2 ring-[#267A4C]/20 shadow-md">
                <AvatarImage src={API_CONFIG.getImageUrl(user?.profilePic) || undefined} alt={user.fullName || "User"} />
                <AvatarFallback className="bg-gradient-to-br from-[#267A4C] to-[#1C5C39] text-white font-semibold">
              ...
              <div className="flex-1 min-w-0 space-y-2">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
                  <h3 className="text-lg font-semibold text-[#142A1C] truncate">{user.fullName || "Unnamed User"}</h3>
                  <Badge variant="secondary" className="bg-[#267A4C]/10 text-[#1C5C39] border-[#267A4C]/20 w-fit">
                    {rolesLoading ? (
                      <div className="flex items-center gap-2">
                        <div className="h-3 w-3 animate-spin rounded-full border border-[#267A4C] border-t-transparent" />
                        Loading...
                      </div>
                    ) : (
                      formatRole(user)
                ...
                <div className="flex flex-wrap gap-4 text-sm text-[#1B4A31]">
                  <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="hidden sm:flex gap-2">
                <Link href={`/admin/users/${user._id}`}>
                  <Button variant="outline" size="sm" className="border-[#267A4C]/40 text-[#267A4C] hover:bg-[#267A4C]/10 hover:border-[#1C5C39] transition-all duration-300">
              ...
              <div className="sm:hidden">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm" className="border-[#267A4C]/40 text-[#267A4C] hover:bg-[#267A4C]/10">
