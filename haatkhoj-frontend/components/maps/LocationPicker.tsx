        ...
        <div className={className}>
            ...
            <div className="mb-3 relative z-20">
                <div className="relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#267A4C]" />
                    <Input
                        type="text"
                        placeholder="Search location (e.g., Richmond Town, Bangalore)"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="pl-10 pr-10 border-[#DCF0E2] focus:border-[#8FD1A3]"
                    />
                    {searchQuery && (
                        <button
                    ...
                    {isSearching && (
                        <Loader2 className="absolute right-10 top-1/2 -translate-y-1/2 h-4 w-4 text-[#267A4C] animate-spin" />
                    )}
                </div>

                {/* Search Results Dropdown */}
                {showResults && searchResults.length > 0 && (
                    <Card className="absolute top-full mt-1 w-full z-50 max-h-60 overflow-y-auto border-[#DCF0E2] shadow-lg bg-white">
                        {searchResults.map((result, index) => (
                            <button
                                key={index}
                                onClick={() => handleSearchResultClick(result)}
                                className="w-full text-left px-4 py-3 hover:bg-[#267A4C]/5 border-b border-[#DCF0E2] last:border-b-0 transition-colors"
                            >
                                <div className="flex items-start gap-2">
                                    <MapPin className="h-4 w-4 text-[#267A4C] mt-1 flex-shrink-0" />
            ...
            <div 
                ref={containerRef}
                className="rounded-2xl overflow-hidden border border-[#DCF0E2] relative z-0"
                style={{ height: `${height}px`, width: '100%', minHeight: `${height}px` }}
            >
                {!mapReady ? (
                    <div className="w-full h-full flex items-center justify-center bg-gray-50">
                        <Loader2 className="h-8 w-8 text-[#267A4C] animate-spin" />
                    </div>
                ) : (
                    <MapContainer
                        center={position}
            ...
            {isLoadingLocation && (
                <p className="text-xs text-[#267A4C] mt-2 flex items-center gap-2">
